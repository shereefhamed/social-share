...
class Social_Share{
    ...
    private function social_share_icons(){
        $output = '';
        $title = isset( Social_Share_Setting::$setting['social_share_title'] )
                ? esc_html__( Social_Share_Setting::$setting['social_share_title'], 'social-share')
                : esc_html__('Share', 'social-sharre'); 
        $services = isset( Social_Share_Setting::$setting['social_share_services'] )
                    ? Social_Share_Setting::$setting['social_share_services'] 
                    :[];
        $shape = isset( Social_Share_Setting::$style['social_share_shape'] ) ? '' : 'rounded';
        ...
        if( !empty( $services ) ){
            foreach( SOCIAL_SHARE_SERVICE as $service){
                ...
                if( in_array( $service['id'], $services ) ){
                    ...
                    $output .= '<a href="'. $link .'" class="social-media-share-icon '. $service['id'] .'" target="_blank">';
                    $output .= '<i class="'. $shape . ' '.$service['icon'] .'"></i>';
                    $output .= '</a>';
                }
            }
        }else{
            $output .= '<p>Please choose social media services</p>';
        }
        $output .= '</div>';
        return $output;
    }
